...
import static com.netflix.karyon.server.http.spi.RequestContextAttributes.queryDecoderKey;
...
public class HttpServletRequestImpl implements HttpServletRequest {
    ...
    @GuardedBy("this") private boolean cookiesParsed; // Don't need to be a volatile as it is always accessed in a synch lock.
    ...
    @VisibleForTesting
    HttpServletRequestImpl(PathComponents pathComponents, FullHttpRequest nettyRequest,
                           HttpSessionManager sessionManager, boolean isSecure) {
        this(pathComponents, nettyRequest, sessionManager, null, isSecure);
    }

    public HttpServletRequestImpl(PathComponents pathComponents, FullHttpRequest nettyRequest,
        ...
        Preconditions.checkNotNull(nettyRequest, "Netty request can not be null,");
    ...
    public /*synchronizes cookies instance creation*/synchronized Cookie[] getCookies() {
        if (null == cookies && !cookiesParsed) {
            ...
            String cookieHeader = nettyRequest.headers().get(COOKIE_HEADER_NAME);
    ...
    public long getDateHeader(String name) {
        return HTTP_DATE_HEADER_HANDLER.getDateAsMillisFromEpoch(nettyRequest.headers(), name);
    }
    ...
    public Enumeration<String> getHeaders(String name) {
        List<String> allValues = nettyRequest.headers().getAll(name);
    ...
    public int getIntHeader(String name) {
        String intHeaderAsString = nettyRequest.headers().get(name);
    ...
    public /*Lazy header parsing to get char encoding hence sync*/ synchronized String getCharacterEncoding() {
        ...
        if (null == contentTypeParser) {
            String contentTypeHeader = nettyRequest.headers().get(HttpHeaders.Names.CONTENT_TYPE);
    ...
    public int getContentLength() {
        ...
        return null != content ? content.readableBytes() : 0;
    }

    @Override
    public String getContentType() {
        return nettyRequest.headers().get(HttpHeaders.Names.CONTENT_TYPE);
    }

    @Override
    public ServletInputStream getInputStream() throws IOException {
        final ByteBufInputStream byteBufIs = new ByteBufInputStream(nettyRequest.content());
    ...
    public Map<String, List<String>> getParameterMap() {
        com.netflix.karyon.server.http.spi.QueryStringDecoder queryStrDecoder =
                getAttributeValueFromContext(queryDecoderKey, channelHandlerContext);
        if (null == queryStrDecoder) {
            queryStrDecoder = new com.netflix.karyon.server.http.spi.QueryStringDecoder(nettyRequest.getUri());
            if (channelHandlerContext != null) {
                channelHandlerContext.attr(queryDecoderKey).setIfAbsent(queryStrDecoder);
            }
        }
    ...
    public BufferedReader getReader() throws IOException {
        return new BufferedReader(new InputStreamReader(new ByteBufInputStream(nettyRequest.content()),
                                                        getCharacterEncodingWithDefault()));
    }
    ...
    public /*Lazy header parsing to get locales hence sync*/ synchronized Enumeration<Locale> getLocales() {
        if (null == languageParser) {
            String acceptLanguageHeader = nettyRequest.headers().get(HttpHeaders.Names.ACCEPT_LANGUAGE);
    ...
    public static class PathComponents {
        ...
        public PathComponents(com.netflix.karyon.server.http.spi.QueryStringDecoder queryStringDecoder,
