...
class ServletsAndFiltersHolder {
    ...
    private final ListMultimap<PipelineDefinition.Key, Class<? extends Filter>> constraintVsFilters =
            Multimaps.newListMultimap(new LinkedHashMap<PipelineDefinition.Key, Collection<Class<? extends Filter>>>(),
    ...
    private final Map<PipelineDefinition.Key, Class<? extends HttpServlet>> constraintVsServlet =
            new HashMap<PipelineDefinition.Key, Class<? extends HttpServlet>>();
    ...
    void configureWithAttacher(HTTPServletRequestRouterBuilder.ServletAndFilterAttacher attacher) {
        PipelineDefinition.Key constraintKey = attacher.getConstraintKey();
     ...
     */
    @Nullable
    ServletMatchResult getMatchingServlet(FullHttpRequest httpRequest, @Nullable ChannelHandlerContext handlerContext) {
        PipelineDefinition.Key.KeyEvaluationContext keyEvaluationContext =
                new PipelineDefinition.Key.KeyEvaluationContext(handlerContext);
        for (Map.Entry<PipelineDefinition.Key, Class<? extends HttpServlet>> servletEntry : constraintVsServlet.entrySet()) {
            PipelineDefinition.Key key = servletEntry.getKey();
            if (key.apply(httpRequest, keyEvaluationContext)) {
                String servletPath = null;
                if (key instanceof ServletStyleUriConstraintKey) {
                    ServletStyleUriConstraintKey servletStyleUriConstraintKey = (ServletStyleUriConstraintKey) key;
    ...
    List<Filter> getMatchingFilters(FullHttpRequest httpRequest, @Nullable ChannelHandlerContext handlerContext) {
        List<Filter> toReturn = new ArrayList<Filter>();
        PipelineDefinition.Key.KeyEvaluationContext keyEvaluationContext =
                new PipelineDefinition.Key.KeyEvaluationContext(handlerContext);
        for (Map.Entry<PipelineDefinition.Key, Collection<Class<? extends Filter>>> filterEntry : constraintVsFilters.asMap().entrySet()) {
