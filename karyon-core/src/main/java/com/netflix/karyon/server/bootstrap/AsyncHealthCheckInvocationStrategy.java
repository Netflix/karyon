 ...
 * {@link #HEALTH_CHECK_TIMEOUT_MILLIS}, this handler will throw a {@link TimeoutException}.
 * The timeout is retrieved using archaius's dynamic properties and hence can be changed at runtime. The default timeout
 * value is as defined by {@link #HEALTH_CHECK_TIMEOUT_DEFAULT} in milliseconds. <p/>
 *
...
public class AsyncHealthCheckInvocationStrategy implements HealthCheckInvocationStrategy {
     ...
     * to be unhealthy.
     */
    public static final String HEALTH_CHECK_TIMEOUT_MILLIS_PROP_NAME =
            PropertyNames.KARYON_PROPERTIES_PREFIX + "health.check.timeout.ms";

    protected static final int HEALTH_CHECK_TIMEOUT_DEFAULT = 1000;

    private static final DynamicIntProperty HEALTH_CHECK_TIMEOUT_MILLIS =
            DynamicPropertyFactory.getInstance().getIntProperty(HEALTH_CHECK_TIMEOUT_MILLIS_PROP_NAME,
                                                                HEALTH_CHECK_TIMEOUT_DEFAULT);

    private static final int TIMEOUT_MILLIS_CONSTANT_NOT_PROVIDED = -1;
    private final int timeoutMillisConstant;
    ...
    public AsyncHealthCheckInvocationStrategy(HealthCheckHandler healthCheckHandler) {
        this(healthCheckHandler, TIMEOUT_MILLIS_CONSTANT_NOT_PROVIDED);
    }

    public AsyncHealthCheckInvocationStrategy(HealthCheckHandler healthCheckHandler, int timeoutMillis) {
        timeoutMillisConstant = timeoutMillis;
    ...
    public int invokeCheck() throws TimeoutException {
        ...
        try {
            return currFuture.get(getTimeoutMillis(), TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
    ...
    private int getTimeoutMillis() {
        if (TIMEOUT_MILLIS_CONSTANT_NOT_PROVIDED != timeoutMillisConstant) {
            return timeoutMillisConstant;
        }
        return HEALTH_CHECK_TIMEOUT_MILLIS.get();
    }
