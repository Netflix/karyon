...
class JsonGrapher extends KaryonAbstractInjectorGrapher {
    ...
    @Inject
    JsonGrapher(NameFactory nameFactory, PortIdFactory portIdFactory, ProvisionMetrics metrics) {
    ...
    protected void postProcess() throws JsonGenerationException, JsonMappingException, IOException {
        metrics.accept(new Visitor() {
            ...
            public void visit(Element element) {
                GraphNode node = nodes.get(element.getKey());
                if (node != null) {
                    node.setDuration(element.getDuration(TimeUnit.MICROSECONDS));
